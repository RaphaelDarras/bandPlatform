...
describe('production build output (dist)', () => {
  const routes = [
    ...
    'contact/index.html',
    'stock/index.html',
    // The internal storefront (/shop, /shop/:id, /cart, /checkout*) moved to
    // Shopify and is fully unmounted, so none of those routes prerender any
    // more — see the "no shell under /shop" assertion below.
  ]
  ...
  it('never emits any static shell under /shop, /cart, or /checkout (storefront moved to Shopify)', () => {
    // The entire in-app commerce surface is unmounted — none of its routes
    // exist to prerender, so none of these directories are baked into dist.
    expect(existsSync(path.join(DIST, 'shop'))).toBe(false)
    expect(existsSync(path.join(DIST, 'cart'))).toBe(false)
    expect(existsSync(path.join(DIST, 'checkout'))).toBe(false)
  })

  it('web/vercel.json no longer carries the /shop rewrite and has no global catch-all', () => {
    const vercelConfig = JSON.parse(
      readFileSync(path.resolve(process.cwd(), 'vercel.json'), 'utf-8'),
    ) as { rewrites?: { source: string; destination: string }[] }
    const rewrites = vercelConfig.rewrites ?? []
    // The /shop/(.*) SPA-fallback rewrite is gone with the storefront.
    expect(rewrites.some((r) => r.source === '/shop/(.*)')).toBe(false)
